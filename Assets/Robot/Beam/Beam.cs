...
public class Beam
{
	...
	public void init(Material material)
	{
		...
		material_.SetColor("_Colors3", new Color(1f, 1f, 0.25f));
#else
	...
	{
		Debug.Assert(alive_table_[id]);
		uv2_list_[id].x = width;
	}
	...
	public int spawn(float width, Type type)
	{
        ...
        public int spawn(float width, float height)
        {
            int cnt = 0;
            while (alive_table_[spawn_index_])
            {
            ...
            alive_table_[spawn_index_] = true;
            int id = spawn_index_;
            uv2_list_[id] = new Vector2(width, height);
            return id;
        }
        ...
        public void renderUpdate(int front, int id, ref Vector3 head, ref Vector3 tail)
	{
		...
		float y = dy*rlen;
		float z = dz*rlen;
		normals_[front][idx+0].x = x;
		normals_[front][idx+0].y = y;
		normals_[front][idx+0].z = z;
		normals_[front][idx+1].x = x;
		normals_[front][idx+1].y = y;
		normals_[front][idx+1].z = z;
		normals_[front][idx+2].x = x;
		normals_[front][idx+2].y = y;
		normals_[front][idx+2].z = z;
		normals_[front][idx+3].x = x;
		normals_[front][idx+3].y = y;
		normals_[front][idx+3].z = z;
