...
public class LoadFriends : MonoBehaviour
{
    ...
    void Load()
    {
        print("Friends :" + GameStateManager.Instance.Friends.Count);
        if (FBGraph.IsFriendListAvailable())
        {
            Destroy(defaultText_FriendList.gameObject);
            foreach (object friend in GameStateManager.Instance.Friends)
            {
    ...
    void LoadScore()
    {
        List<object> friends = GameStateManager.Instance.Friends;
