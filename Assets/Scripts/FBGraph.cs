...
public static class FBGraph
{
    ...
    private static void CacheFriends(IList<object> newFriends)
    {
        ...
        {
            GameStateManager.Instance.Friends.AddRange(newFriends);
        }
        else
        {
            GameStateManager.Instance.Friends = (List<object>)newFriends;
        }
    }

    public static bool IsFriendListAvailable()
    {
        return GameStateManager.Instance.Friends != null && GameStateManager.Instance.Friends.Count > 0;
    }
    #endregion
    ...
    private static void HandleScoresData(List<object> scoresList)
    {
        ...
        foreach (object scoreItem in scoresList)
        {
            ...
            if (!GameStateManager.Instance.FriendImages.ContainsKey(userId))
            {
                ...
                LoadFriendImgFromID(userId, pictureTexture =>
               {
                   if (pictureTexture != null)
                   {
                       GameStateManager.Instance.FriendImages.Add(userId, pictureTexture);
                   }
               });
            }
        }
